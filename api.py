...
def _sanitize_for_log(html: str) -> str:
    """Verwijder e-mails/gebruikersnamen uit logsnippets."""
    html = re.sub(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", "[redacted@mail]", html)
    return html
...
def _find_value_before_label(soup: BeautifulSoup, label_regex: str) -> Optional[int]:
    ...
    if not label:
        return None
    # Zoek vorige tekstnode met alleen cijfers/komma's/punten
    prev = label.find_previous(string=re.compile(r"^[\d\.,]+$"))
    if prev:
        return _clean_number(prev)
    # Fallback: in parenttekst
...
def _parse_stats_from_html(html: str) -> Tuple[Optional[int], Optional[int]]:
    """Retourneer (lifetime, season) uit /log HTML.
    We zoeken expliciet naar labels en pakken het getal direct ervoor.
    """
    ...
    # Regex fallbacks als labels in vrije tekst staan
    if lifetime is None:
        m = re.search(r"([\d\.,]+)\s*Lifetime\s*Meters", html, re.I)
        if m:
            lifetime = _clean_number(m.group(1))
    if season is None:
        m = re.search(r"([\d\.,]+)\s*Season\s*Meters", html, re.I)
        if m:
            season = _clean_number(m.group(1))
...
class Concept2Client:
    ...
    async def _get_csrf(self) -> Optional[str]:
        try:
            ...
            async with self._session.get(
                ...
                if resp.status != 200:
                    _LOGGER.warning("Concept2: CSRF-pagina gaf status %s", resp.status)
                    return None
                html = await resp.text()
        except Exception as exc:
            _LOGGER.warning("Concept2: fout bij ophalen loginpagina: %s", exc)
            return None
        ...
        _LOGGER.warning("Concept2: CSRF-token niet gevonden op loginpagina")
        return None

    async def login(self) -> bool:
        token = await self._get_csrf()
        if not token:
            _LOGGER.warning("Concept2: login overgeslagen (geen CSRF-token)")
            return False
        ...
        try:
            ...
            async with self._session.post(
                ...
                if resp.status not in (200, 302):
                    _LOGGER.warning("Concept2: login mislukte: HTTP %s", resp.status)
                    return False
        except Exception as exc:
            _LOGGER.warning("Concept2: login call exception: %s", exc)
            return False
    ...
    async def fetch_stats(self) -> Dict[str, Optional[int]]:
        """Haal lifetime & season meters op. Retourneer None-waarden bij storingen."""
        ...
        for url in DASHBOARD_CANDIDATES:
            try:
                ...
                async with self._session.get(
                    url, headers={"User-Agent": USER_AGENT, "Referer": LOGIN_URL}, allow_redirects=True
                ) as resp:
                    if resp.status in (401, 403):
                        ...
                        continue
                    if resp.status != 200:
                        last_error = f"HTTP {resp.status} op {url}"
                        _LOGGER.debug("Concept2: %s", last_error)
                        continue
                    html = await resp.text()
                ...
                _LOGGER.debug(
                    "Concept2: parsing mislukt op %s, snippet: %s",
                    url,
                    _sanitize_for_log((html or "")[:800]),
                )
                last_error = "Parser vond geen meters"
            except ClientResponseError as cre:
                last_error = f"HTTP error {cre.status} op {url}"
                _LOGGER.debug("Concept2: %s", last_error)
            except Exception as exc:
                last_error = f"Fout op {url}: {exc}"
                _LOGGER.debug("Concept2: %s", last_error)
